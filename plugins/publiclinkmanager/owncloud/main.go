...
func RegisterConfig(gc *goconfig.GoConfig) {
	gc.Add("publiclinkmanager-plugin-db-host", "/eos/scratch/user/", "the prefix that will be added to paths.")
	gc.Add("publiclinkmanager-plugin-db-port", "/usr/bin/eos", "the eos client binary.")
	gc.Add("publiclinkmanager-plugin-db-username", "/usr/bin/xrdcopy", "the xrdcopy binary.")
	gc.Add("publiclinkmanager-plugin-db-password", "root://example.org", "the url of the mgm node.")
	gc.Add("publiclinkmanager-plugin-db-name", "", "directory to store cached information. Empty defaults to OS temporary directory.")
}

func New(gc *goconfig.GoConfig, logger *zap.Logger, vfs api.VirtualStorage) (api.PublicLinkManager, error) {
	dbUsername := gc.GetString("publiclinkmanager-plugin-db-username")
	dbPassword := gc.GetString("publiclinkmanager-plugin-db-password")
	dbHost := gc.GetString("publiclinkmanager-plugin-db-host")
	dbPort := gc.GetInt("publiclinkmanager-plugin-db-port")
	dbName := gc.GetString("publiclinkmanager-plugin-db-name")
